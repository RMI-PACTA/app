// Command dashboard is our shim for generating dashboard for PACTA porfolios,
// wrapping ghcr.io/rmi-pacta/workflow.pacta.dashboard
package main
...
func run(args []string) error {
	...
	req, err := async.LoadCreateDashboardRequestFromEnv()
	if err != nil {
	...
	if err := h.CreateDashboard(ctx, taskID, req, *azDestPortfolioContainer); err != nil {
		return fmt.Errorf("error running task: %w", err)
	}

	logger.Info("ran PACTA dashboard task successfully", zap.String("task_id", string(taskID)))
