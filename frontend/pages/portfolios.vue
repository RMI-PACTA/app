...
const selectedRows = useState(`${prefix}.selectedRows`, () => [])
...
const addToPortfolioGroup = (pid: string, gid: string) => withLoading(
  () => pactaClient.addPortfolioToGroup(id, gid),
  `${prefix}.addToPortfolioGroup`,
)
const addToInitiative = (pid: string, iid: string) => withLoading(
  () => pactaClient.addPortfolioToInitiative(id, iid),
  `${prefix}.addToInitiative`,
)
</script>
